...
public class LocketteProAPI {
    ...
    public static BlockFace[] allfaces = { BlockFace.NORTH, BlockFace.EAST, BlockFace.SOUTH, BlockFace.WEST,
        ...
        switch (block.getType()) {
                ...
                for (BlockFace doorface : newsfaces) {
                    ...
                    if (relative0.getType() == doors[0].getType() && relative1.getType() == doors[1].getType()) {
                        if (isLockedSingleBlock(relative1.getRelative(BlockFace.UP), doorface.getOppositeFace()))
                            return true;
                        if (isLockedSingleBlock(relative1, doorface.getOppositeFace()))
                            return true;
                        if (isLockedSingleBlock(relative0, doorface.getOppositeFace()))
                            return true;
                        if (isLockedSingleBlock(relative0.getRelative(BlockFace.DOWN), doorface.getOppositeFace()))
                            return true;
                    }
                }
                if (isLockedSingleBlock(doors[1].getRelative(BlockFace.UP), null))
                    return true;
                if (isLockedSingleBlock(doors[1], null))
                    return true;
                if (isLockedSingleBlock(doors[0], null))
                    return true;
                if (isLockedSingleBlock(doors[0].getRelative(BlockFace.DOWN), null))
                    return true;
                break;
            case LECTERN:
            ...
            case TRAPPED_CHEST:
                ...
                if (chestface != null) {
                    ...
                    if (isLockedSingleBlock(relativechest, chestface.getOppositeFace()))
                        return true;
                }
                // Don't break here
                // Everything else (First block of container check goes here)
            default:
                if (isLockedSingleBlock(block, null))
                    return true;
                break;
        }
        return false;
        ...
        switch (block.getType()) {
            ...
            case IRON_DOOR:
                ...
                for (BlockFace doorface : newsfaces) {
                    Block relative0 = doors[0].getRelative(doorface), relative1 = doors[1].getRelative(doorface);
                    if (relative0.getType() == doors[0].getType() && relative1.getType() == doors[1].getType()) {
                        String f1 = getLockOwnerSingleBlock(relative1.getRelative(BlockFace.UP),
                                doorface.getOppositeFace());
                        ...
                        String f4 = getLockOwnerSingleBlock(relative0.getRelative(BlockFace.DOWN),
                                doorface.getOppositeFace());
                        if (f1 != null) {
                            return f1;
                        } else if (f2 != null) {
    ...
    public static boolean isOwner(Block block, Player player) {
        switch (block.getType()) {
                ...
                for (BlockFace doorface : newsfaces) {
                    ...
                    if (relative0.getType() == doors[0].getType() && relative1.getType() == doors[1].getType()) {
                        if (isOwnerSingleBlock(relative1.getRelative(BlockFace.UP), doorface.getOppositeFace(), player))
                            return true;
                        if (isOwnerSingleBlock(relative1, doorface.getOppositeFace(), player))
                            return true;
                        if (isOwnerSingleBlock(relative0, doorface.getOppositeFace(), player))
                            return true;
                        if (isOwnerSingleBlock(relative0.getRelative(BlockFace.DOWN), doorface.getOppositeFace(),
                                player))
                            return true;
                    }
                }
                if (isOwnerSingleBlock(doors[1].getRelative(BlockFace.UP), null, player))
                    return true;
                if (isOwnerSingleBlock(doors[1], null, player))
                    return true;
                if (isOwnerSingleBlock(doors[0], null, player))
                    return true;
                if (isOwnerSingleBlock(doors[0].getRelative(BlockFace.DOWN), null, player))
                    return true;
                break;
            // Chests (Second block only)
            case LECTERN:
            ...
            case TRAPPED_CHEST:
                ...
                if (chestface != null) {
                    Block relativechest = block.getRelative(chestface);
                    if (isOwnerSingleBlock(relativechest, chestface.getOppositeFace(), player))
                        return true;
                }
                // Don't break here
                // Everything else (First block of container check goes here)
            default:
                if (isOwnerSingleBlock(block, null, player))
                    return true;
                break;
        }
        return false;
    ...
    public static boolean isUser(Block block, Player player) {
        switch (block.getType()) {
                ...
                for (BlockFace doorface : newsfaces) {
                    ...
                    if (relative0.getType() == doors[0].getType() && relative1.getType() == doors[1].getType()) {
                        if (isUserSingleBlock(relative1.getRelative(BlockFace.UP), doorface.getOppositeFace(), player))
                            return true;
                        if (isUserSingleBlock(relative1, doorface.getOppositeFace(), player))
                            return true;
                        if (isUserSingleBlock(relative0, doorface.getOppositeFace(), player))
                            return true;
                        if (isUserSingleBlock(relative0.getRelative(BlockFace.DOWN), doorface.getOppositeFace(),
                                player))
                            return true;
                    }
                }
                if (isUserSingleBlock(doors[1].getRelative(BlockFace.UP), null, player))
                    return true;
                if (isUserSingleBlock(doors[1], null, player))
                    return true;
                if (isUserSingleBlock(doors[0], null, player))
                    return true;
                if (isUserSingleBlock(doors[0].getRelative(BlockFace.DOWN), null, player))
                    return true;
                break;
            // Lecterns
            case LECTERN:
            ...
            case TRAPPED_CHEST:
                ...
                if (chestface != null) {
                    Block relativechest = block.getRelative(chestface);
                    if (isUserSingleBlock(relativechest, chestface.getOppositeFace(), player))
                        return true;
                }
                // Don't break here
                // Everything else (First block of container check goes here)
            default:
                if (isUserSingleBlock(block, null, player))
                    return true;
                break;
        }
        return false;
    ...
    public static boolean isLockedSingleBlock(Block block, BlockFace exempt) {
        for (BlockFace blockface : newsfaces) {
            ...
            if (isLockSign(relativeblock) && getFacing(relativeblock) == blockface) {
                // Found [Private] sign, is expire turned on and expired? (relativeblock is now
                // sign)
    ...
    public static boolean isOwnerOfSign(Block block, Player player) { // Requires isSign
        ...
        if (isOwner(protectedblock, player))
            return true;
        // Situation where double door's block
        if (isUpDownLockedDoor(protectedblock) && isOwnerUpDownLockedDoor(protectedblock, player))
            return true;
        // Otherwise...
        return false;
    }
    ...
    public static boolean isLockable(Block block) {
        ...
        if (Config.isLockable(material)) { // Directly lockable
            ...
            if (blockup != null && isUpDownAlsoLockableBlock(blockup))
                return true;
            Block blockdown = block.getRelative(BlockFace.DOWN);
            if (blockdown != null && isUpDownAlsoLockableBlock(blockdown))
                return true;
            return false;
        }
    }
    ...
    public static boolean mayInterfere(Block block, Player player) {
        ...
        switch (block.getType()) {
            ...
            case DISPENSER:
            case DROPPER:
                if (!Config.isInterferePlacementBlocked())
                    return false;
                for (BlockFace blockface : allfaces) {
                    Block newblock = block.getRelative(blockface);
                    switch (newblock.getType()) {
    ...
    public static boolean isOwnerOnSign(Block block, Player player) { // Requires isLockSign
        String[] lines = Utils.getSignLines((Sign) block.getState());
    ...
    public static boolean isUserOnSign(Block block, Player player) { // Requires (isLockSign or isAdditionalSign)
        ...
        for (int i = 1; i < 4; i++) {
            if (Dependency.isPermissionGroupOf(lines[i], player))
                return true;
            if (Dependency.isScoreboardTeamOf(lines[i], player))
                return true;
        }
        return false;
    }
    ...
    public static boolean isUpDownLockedDoor(@Nullable Block block) {
        ...
        Block blockup = block.getRelative(BlockFace.UP);
        if (blockup != null && isUpDownAlsoLockableBlock(blockup) && isLocked(blockup))
            return true;
        Block blockdown = block.getRelative(BlockFace.DOWN);
        if (blockdown != null && isUpDownAlsoLockableBlock(blockdown) && isLocked(blockdown))
            return true;
        return false;
    }
    ...
    public static boolean isOwnerUpDownLockedDoor(Block block, Player player) {
        Block blockup = block.getRelative(BlockFace.UP);
        if (blockup != null && isUpDownAlsoLockableBlock(blockup) && isOwner(blockup, player))
            return true;
        Block blockdown = block.getRelative(BlockFace.DOWN);
        if (blockdown != null && isUpDownAlsoLockableBlock(blockdown) && isOwner(blockdown, player))
            return true;
        return false;
    }
    ...
    public static boolean isUserUpDownLockedDoor(Block block, Player player) {
        Block blockup = block.getRelative(BlockFace.UP);
        if (blockup != null && isUpDownAlsoLockableBlock(blockup) && isUser(blockup, player))
            return true;
        Block blockdown = block.getRelative(BlockFace.DOWN);
        if (blockdown != null && isUpDownAlsoLockableBlock(blockdown) && isUser(blockdown, player))
            return true;
        return false;
    }
    ...
    public static int getTimerOnSigns(Block block) {
        for (BlockFace blockface : newsfaces) {
            ...
            if (isSign(relative)) {
                ...
                for (String line : Utils.getSignLines(sign)) {
    ...
    public static int getTimerSingleDoor(Block block) {
        ...
        if (relativedowntimer > 0)
            return relativedowntimer;
        return 0;
    }
